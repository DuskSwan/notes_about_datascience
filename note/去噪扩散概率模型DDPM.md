...
说明：公式中向量的粗体和普通体，也即$\mathbf{x_t}$和$x_t$是混用的，因为原论文用的是粗体，而我敲公式懒得用，日后如果有空会改统一。不管是否加粗，变量都应该是多元的，也就是向量。
...
考虑这样一件事，只需要得到一个服从$N(0,I)$的样本$z$，然后做变换$x=Az+\mu$，就得到了服从$N(\mu,AA')$的样本$x$。这启示我们，$x_t$可以由服从$N(0,I)$的样本$z$做变换$x_t=\sqrt{\beta_t}Iz+\sqrt{1-\beta_t}x_{t-1}$得到。（所谓的）
...
> 我认为这里计算的是$x_t$的某一分量，因为其各个分量相互独立，所以每个分量的计算方式都一样，也可以用分量的计算代表整体。这里的$q$当作密度函数来理解，分量的方差是标量而不是矩阵，套用贝叶斯公式在连续型变量下的公式即可。
...
> 虽然前面提到了这是按照封闭形式（closed form）计算的，但是好像和公式不完全一样吧( ╯□╰ )这个推导暂时存疑。我也怀疑这里是因为分量相互独立，所以用分量也就是一元变量来做计算，但这样又说不通为什么$\Sigma$的范数要带下标$2$了。
