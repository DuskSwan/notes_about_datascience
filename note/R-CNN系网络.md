...
RoI pooling的目的是将原图缩放到统一的大小。在RoI pooling之前，得到的应该是一系列选框（称为RoI, region of interest，或者叫Proposal），每个RoI实际是五维向量(id,x,y,w,h)。此外还有全图的特征图（原图尺寸*多个通道）。

通过在特征图上按照RoI的(x,y,w,h)划出区域，就得到了属于该RoI的投影，一个多通道的RoI特征图。这一步称为RoI投影。

> 举个例子，比如通过RPN得到了一个Proposal，它在原图上的左上角坐标是(10,10)，右下角的坐标是(124,124)，对应要映射的特征层相对原图的步距为32。那么要在特征图上抠出来的区域左上角就是(0,0)，右下角是(4,4)，这分别是由10/32=0和124/32=4四舍五入而来。
...
> 继续刚才的例子，假如通过RoIPool期望的输出为2x2大小，那么就要被抠出来的(0,0)至(4,4)的特征图再分成2×2的四部分，这时边长实际为5，需要再次四舍五入，划分时边长为3和2。现在，才能在每个小区域内找最大值（进行最大值池化）。
...
该图来自[陈云](https://zhuanlan.zhihu.com/p/32404424)。是编程角度的流程图。
...
RPN的输入是原图特征图（在卷积的作用下会有多个），输出多个候选框坐标。原始图片的特征图记为F1，形状为(c,w,h)，c是特征图个数。在F1上使用$3\times3$的卷积得到(512,w,h)的特征图F2，现在F2上的一个点(x,y)代表原图的一个区域，它拥有一个512维的向量作为特征。（值得说明的是，这里的512其实是backbone的通道数，如果用VGG16为512个通道，ZF网络则是256个通道）

对于F2上的一个点(x,y)，我们考虑以他为中心的$k$个不同大小的锚框（anchor box）。在论文中，锚框的面积为{128^2^,256^2^,512^2^}，长宽比为{0.5,1,2}，实际用了k=3*3=9个锚框。对特征图上的每一个点都要考虑如此的多个锚框，对于大小为h×w=K的特征图，实际得到K×9个框，计算出它们在原图上的坐标。

使用点(x,y)的256维特征向量去计算2个分数（前景or背景）和4个坐标（RoI的坐标参数），共得到$6k$个值。每个anhcor要分positive和negative（理解为前景和背景），之后训练时在合适的anchors中随机选取128个postive anchors+128个negative anchors进行训练。
