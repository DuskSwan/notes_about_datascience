...
GPT的训练方式被称为生成式预训练，目标是根据上文$C=\{x_1,...,x_{t-1}\}$，预测出下一个单词$x_t$。要实现这一步，本质上是需要学习出分布$P(x_t|C)$。假如设定$x_0=$`<sos>`为起始符，那么一个句子$x=\{x_1,\cdots,x_n,\cdots\}$的分布$P(x)=\prod P(x_t|C)$都可以求出。
