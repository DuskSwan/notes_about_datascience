...
这一步根据每个点(x,y)、每个锚框的(w,h,2k)与(w,h,4k)的“特征”，计算出RoI，代码层面使用ProposalCreator完成。

目前我们有w×h个点，每个点k个锚框。值得注意的是，计算出修正值后，立刻将修正值应用于锚框，此时锚框的大小已经和设定的“标准”不同了，此时把他们称为候选RoI更合适。候选RoI总数应该是w×h×k这么多。
...
>
> 为什么修正值不是直接作为系数乘或者加在原本的$p_h,p_w$等等上面，我也不知道，推测是为了控制修正值的幅值不要太大。但不得不说这个修正不太直观。
...
> 这里应该有AnchorTargerCreator的作用，但我还没懂
...
RPN的损失分又为两个部分，分类损失和边界框回归损失。它描述了RPN网络在捕捉前景和定位上的性能。
$$
